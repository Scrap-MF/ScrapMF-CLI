...
fn resolve_via_gallery_dl(
    ...
    let url = format!("https://www.facebook.com/profile.php?id={id}");
    ...
    // Also try to parse "user" or "profile" variations
    for line in stdout.lines() {
        // Look for any line that looks like a username after ID resolution
        // Facebook gallery-dl -K may output "user" instead of "username" in some cases
        if line.trim() == "user" {
            if let Some(val) = lines.next() {
                let v = val.trim();
                if !v.is_empty() {
